 ...
 *
 * The previous quality gate used the remote W3C validator through
 * html-validator-cli. In CI/proxy-restricted environments that makes
 * `npm test` fail before checking the app. These checks cover the static
 * invariants this repo depends on: valid entry documents, unique IDs,
 * balanced common tags and existing local assets.
 */
'use strict';
...
function lintHtmlFile(relPath) {
  ...
  assert(/<html\s+lang="(lt|en)"/i.test(html), `${relPath}: has supported html lang`);
