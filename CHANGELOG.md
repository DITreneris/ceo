...
- **CI `lint:html` (W3C validator 403)** — GitHub Actions failed when `html-validator-cli` called `validator.w3.org` (HTTP 403 from CI IPs). Added `scripts/lint-html.js`: tries `html5.validator.nu` fallback, then skips online validation in `CI` only with a warning (structure tests + ESLint still run). Local `npm run lint:html` unchanged when W3C is reachable.
